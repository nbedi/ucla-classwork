...
#define MAXIMUM_FILE_SIZE 1024*20					
//avoid slow peers
...
#define TASKBUFSIZ	1024*64	// Size of task_t::buf
...
task_t *start_download(task_t *tracker_task, const char *filename)
{
	...
	strncpy(t->filename, filename, FILENAMESIZ); //changed to strncpy
	t->filename[FILENAMESIZ-1]='\0'; //buffer overrun fix, file name (2A)
...
* Try 'rm %s.~*~' to remove them.\n", t->filename, t->filename);
	...
	while (1) {
		...
		if (t->total_written > MAXIMUM_FILE_SIZE) {
			error("File exceeds MAXIMUM_FILE_SIZE.\n");
			goto try_again;
		}
	}
...
static void task_upload(task_t *t)
{
	...
	//exercise 2b: peer should not serve files outside current directory
	char requested_path_buf[PATH_MAX+1];
	char* requested_path = realpath(t->filename, requested_path_buf);
	char current_path_buf[PATH_MAX+1];
	char* current_path = getcwd(current_path_buf, PATH_MAX + 1);

	if (strncmp(current_path, requested_path, strlen(current_path))) {
		errno = ENOENT;
	...
	while (1) {
		...
		int ret;
		if(evil_mode) {
			write(t->peer_fd, t->buf, TASKBUFSIZ-1);
		}
		else {
